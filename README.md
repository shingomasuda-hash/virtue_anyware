...
| [docs/15_DEAL_MANAGEMENT.md](docs/15_DEAL_MANAGEMENT.md) | **案件管理（太陽光・蓄電池）**・報酬計算・進捗管理（現行スプレッドシートの移行） |
...
| URL | 権限 | 内容 |
| --- | --- | --- |
| `/users` | `user:manage`（本部管理者以上） | ユーザー一覧（氏名 / ロール / 所属代理店 / 最終ログイン / 有効・無効） |
| `/users/new` | `user:manage` | 登録（ロール・所属代理店・初期パスワード） |
| `/users/[id]/edit` | `user:manage` | 編集 + 管理者によるパスワード再設定 |
| `/account` | ログイン済みの本部ユーザー | 本人によるパスワード変更 |
| `/agency/account` | ログイン済みの代理店ユーザー | 本人によるパスワード変更 |

サーバー側のガード（`src/server/services/users.ts`。画面の出し分けだけに頼らない）:

- `SUPER_ADMIN` の作成・変更・パスワード再設定は `SUPER_ADMIN` のみ（権限昇格の防止）
- 代理店ロールは自組織の代理店の指定が必須。本部ロールには代理店を紐づけない
- 自分自身の無効化・自分のロール変更は不可
- 組織の有効な管理者が 0 人になる操作は不可
- 一覧・詳細・更新・再設定のすべてを `organizationId` で絞り込み（ID 直接指定でも他組織へ到達できない）
- 無効化・パスワード再設定・パスワード変更は対象セッションを失効させる
- 作成 / ロール変更 / パスワード再設定 / パスワード変更はすべて監査ログに残る

パスワードポリシー（12文字以上・3種類以上。判定は `src/lib/password.ts` の 1 箇所）は
画面・Server Action・`npm run db:bootstrap` が同じ実装を参照する。

未実装 PHASE のメニューはサイドバーで「準備中」と表示し、リンクにしていない
（存在しない URL へ遷移させないため）。
...
現行の Google スプレッドシート「顧客・案件管理シート」をシステムへ移したもの。
電力契約（`contracts`）とは**金額の決まり方が違う**ため別エンティティ `deals` として持つ。

| | 電力契約 | 案件 |
| --- | --- | --- |
| 金額 | 単価マスタから解決（W数・使用量 × 単価） | **案件ごとの販売価格と原価** |
| 代理店への支払 | 単価ベース | **歩合（コミッション）** |
...
| URL | 権限 | 内容 |
| --- | --- | --- |
| `/deals` | `deal:read` | 一覧（ステータス / 商材 / 代理店 / 営業 / 進行中 / 期限超過で絞り込み）＋ KPI |
| `/deals/pipeline` | `deal:read` | ファネル（ステータス別の件数・構成比・販売価格合計） |
| `/deals/new` `/deals/[id]/edit` | `deal:write` | 案件の登録・編集（案件IDは空欄で自動採番） |
| `/deals/[id]` | `deal:read` | 詳細（概要 → 進捗 → 報酬 → 対応履歴） |
| `/deals/[id]/progress` | `deal:progress` | 審査・現調・補助金・工事・入金・書類の更新 |
| `/deals/[id]/compensation` | `deal:compensation` | 原価・控除額・率の入力。計算結果を即時表示 |
| `/agency/deals` `/agency/deals/[id]` | `deal:read` | 代理店向け（自社案件の閲覧のみ・報酬なし） |

### 報酬計算（`src/server/services/deals/compensation.ts` の 1 箇所だけで行う）
...
`max(0, …)` により**赤字案件ではコミッションが 0** になり、損失は会社残粗利に残る。
計算結果は列として保存するため、率や控除額の運用を変えても過去の支払額は動かない。

### 認可（§4 / §17）

- 代理店ロールは `agencyScope()` により自代理店の案件のみ。**自社直販案件（`agencyId = NULL`）は 1 件も見えない**
- **報酬は本部管理者のみ**（`deal:compensation`）。原価・営業利益・控除額・会社残粗利・営業コミッションは代理店へ返さない
- 代理店ロールの KPI では「営業利益合計」を `null` で返す
- 案件の作成・更新・ステータス変更・進捗更新・報酬更新はすべて監査ログに記録

### デモデータ

`prisma/seed/data/*.csv`（現行シートの書き出し。**すべて架空のダミー**）から
顧客 40 件・案件 40 件・進捗 21 件・報酬 21 件をシードする。
報酬は CSV の計算済み列を写さず、入力値から計算し直して保存するため、シードが計算の実地検証にもなる。

シートのダッシュボードと一致することを確認済み:
総顧客数 40 / 進行中 25 / 今月の契約 23 件 / 今月の契約売上 75,380,000 / 営業利益合計 19,780,000 /
次回アクション超過 14（基準日 2026-09-23）/ 入金期限超過 0 / 工事待ち 2。
