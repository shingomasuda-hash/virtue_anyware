...
## 7.5 戻入（クローバック）を織り込んだ粗利

条件表の戻入条件により、粗利は「確定」と「未確定」に分かれる。
経営数値を過大に見せないため、両方を並記する。

```
確定粗利       = Σ hq_gross_profit（戻入リスクが RELEASED の契約）
リスク保有粗利  = Σ hq_gross_profit（戻入リスクが AT_RISK の契約）
戻入確定額     = Σ hq_revenue（戻入が CONFIRMED の契約）
リスク込み粗利  = 確定粗利 + リスク保有粗利 − 戻入確定額
```

| 状態 | 意味 |
| --- | --- |
| `AT_RISK` | 期限内。条件を満たさなければ返還義務が生じる |
| `CONFIRMED` | 戻入が確定した（返還対象） |
| `RELEASED` | 期限を過ぎ、返還義務が消滅した |
